...
export class EnrichmentMicroserviceController {
    ...
    async enrichPeople(data: any) {
        try
        {
            let { firstName, lastName, email, phoneNumber, domain, provider,linkedin_url } = data;
            ...
            return await this.enrichmentService.getPeople({linkedin_url:linkedin_url},provider);
            }
            else {
