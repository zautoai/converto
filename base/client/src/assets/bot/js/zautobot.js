...
function initialize() {
    ...
    function initUIComponent() {
        ...
        if (rootElement) {
            ...
            chatBot.classList.add(isStandalone ? "middle_center" : ( avatarData.position.toLocaleLowerCase() || "bottom_center"));
            ...
            // setTimeout(() => {
            //     generateCTA({name:"Signup",description:"Free signup for 14 days free trial.",type:"CTA",link:"https://zauto.ai"});
            // }, 10000);
            ...
            if(!isStandalone)
            {
                ...
                setTimeout(() => {
                    if(!isActive)
                    {
                        // toggleChatBot(true,null);
                        messageInput.focus();
                    }
                }, avatarData.wakeupTime || 5000);
            }
        }
    }
    ...
    function toggleChatBot(_isActive, eventArg) {
        if(eventArg && eventArg.type == "blur" && eventArg.relatedTarget)
        {
            return;
        }
        isActive = _isActive;
    ...
    async function addMessage(id,markdownText, isUser = false, sendByName = "Blink", doAnimation = true, timestamp = null,vote=null) {
        ...
        if(!isUser)
        {
            ...
            imageContainer.className = "zauto_avatar_image";

            const img = document.createElement("img");
            img.src = avatarData.logoUrl || baseUrl + "assets/bot/imgs/default_avatar.png";
            imageContainer.appendChild(img);
            messageframe.appendChild(imageContainer);
        }
    ...
    function generatePoweredBy(parent) {
        ...
        poweredByImage.src = "../assets/bot/imgs/votbot_logo .png";
    ...
    async function playPopSound(audioUrl) {
        if (audioUrl) {
