...
@Component({
  selector: 'app-abmview',
  templateUrl: './abmview.component.html',
  styleUrl: './abmview.component.scss'
})
export class AbmviewComponent {
  calltoaction() {
  ...
  ngOnInit(): void {
    this.abmId = this.route.snapshot.paramMap.get('id') ?? '';
    this.fetchUserData(this.abmId);

  }

  fetchUserData(id: string) {
    this.restService.get(API.main.abm, id).subscribe(
      (response: any) => {
