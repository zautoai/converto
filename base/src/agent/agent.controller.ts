import { Body, ConflictException, Controller, Delete, Get, Header, HttpCode, HttpException, HttpStatus, NotFoundException, Param, Patch, Post, Query, Req, UnauthorizedException, UploadedFile, UseGuards, UseInterceptors } from '@nestjs/common';
...
export class AgentController {
  ...
  @UseGuards(SubdomainGuard)
  async embedding(@Param('agentId') agentId: string, @Req() request: SubdomainRequest) {
    const orgId = request.orgId;
    if (agentId.includes('.js')) {
      agentId = agentId.replace('.js', '');
      return await this.agentsService.getEmbedding(orgId, agentId);
    }
    else {
