...
export class DashboardService {
    ...
    async getAgentsCount(orgId: string) {
        try {
            return await this.prisma.agent.count({ where: { orgId, status: { not: AgentStatus.DELETED } } })
        } catch (error) {
            console.log(error)
        }
    }
    ...
    async getMessagesCount(orgId: string) {
        try {
            const conversations = await this.prisma.zautoMessage.count({ where: { orgId, role: 'assistant', type: 'TEXT' } });
    ...
    async getMessagesCountByAgent(agentId: string) {
        try {
            const conversations = await this.prisma.zautoMessage.count({ where: { agentId, role: 'assistant', type: 'TEXT' } });
            return conversations;
        } catch (error) {
            console.log(error)
        }
    ...
    async getConversationCountByAgent(agentId: string) {
        try {
            const conversations = await this.prisma.conversation.count({ where: { agentId } });
            return conversations;
        } catch (error) {
            console.log(error)
        }
    ...
    calculateDateRange(dashbaordDto: DashbaordDto): { startDate: Date; endDate: Date } {
        ...
        switch (dateFilter) {
            ...
            case DateFilter.THIS_WEEK:
                ...
                calculatedEndDate = new Date(now.setDate(diff + 6));
                calculatedEndDate.setHours(23, 59, 59, 999);
                break;
            case DateFilter.BETWEEN:
                if (!startDate || !endDate) {
                    throw new BadRequestException('Start date and end date are required for BETWEEN filter');
                }
        ...
        const startDateString = calculatedStartDate.toISOString().slice(0, 10) + 'T00:00:00.000Z';
        const endDateString = calculatedEndDate.toISOString().slice(0, 10) + 'T23:59:59.999Z';
    ...
    async getCounts(orgId: string,dashbaordDto:DashbaordDto)
    {
        ...
        const convoData = await this.getConversationCountByDate(orgId,dashbaordDto);
        const campaignData = await this.getCampaignCountByDate(orgId,dashbaordDto);        

        const visitorCount = visitorData.values.reduce((acc, curr) => acc + curr, 0);
        const leadCount = leadData.values.reduce((acc, curr) => acc + curr, 0);
        const convoCount = convoData.values.reduce((acc, curr) => acc + curr, 0);
        const campaignCount = campaignData.values.reduce((acc, curr) => acc + curr, 0);
        return {
            visitorCount,
            leadCount,
            convoCount,
    ...
    calculateCampaignScore(campaign) {
        ...
        const score = (leadCount * leadWeight) + (convoCount * convoWeight) + (visitCount * visitWeight);
