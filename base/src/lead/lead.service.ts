...
export class LeadService {
  ...
  async create(createLeadDto: CreateLeadDto, orgId: string) {
    try {
      const lead = await this.prisma.lead.findFirst({where: {convId: createLeadDto.convId}});
      if(lead) {
        ...
        if(createLeadDto.email) {
          const isValidEmail = await this.emailService.validateEmailDomain(createLeadDto.email);
          if(!isValidEmail) {
            throw new BadRequestException(`The email address "${createLeadDto.email}" is invalid or not allowed.`);
          }
          const _existingLead = await this.prisma.lead.findFirst({where: {email: createLeadDto.email, orgId}});
          if(_existingLead && _existingLead.convId != createLeadDto.convId) {
            ...
            await this.prisma.zautoMessage.updateMany({ where: {convId: _existingLead.convId }, data: { convId: createLeadDto.convId}})
            createLeadDto = {..._existingLead, ...leadcopy, email: leadcopy.email};
            await this.prisma.conversation.delete({where: {id: _existingLead.convId}});
        ...
        return await this.prisma.lead.update({data: {...createLeadDto, orgId}, where:{id: lead.id}})
      } else {
        ...
        if(createLeadDto.email) {
          ...
          let leadcopy = JSON.parse(JSON.stringify(createLeadDto));
          const _existingLead = await this.prisma.lead.findFirst({where: {email: createLeadDto.email}});
          if(_existingLead && _existingLead.convId != createLeadDto.convId) {
            await this.prisma.zautoMessage.updateMany({ where: {convId: _existingLead.convId }, data: { convId: createLeadDto.convId}})
            createLeadDto = {..._existingLead, ...leadcopy, email: leadcopy.email};
            await this.prisma.conversation.delete({where: {id: _existingLead.convId}});
        ...
        return await this.prisma.lead.create({data: {...createLeadDto, orgId, crmContactid}});
      }
    } catch(error) {
  ...
  async update(id: string, orgId: string, updateLeadDto: UpdateLeadDto) {
    ...
    if(existingLead) {
      ...
      return await this.prisma.lead.update({data: updateLeadDto, where:{id}});
    } else throw new NotFoundException(`Lead with id ${id} not found.`);
  }
  ...
  async validateEmail(validateEmailDto:ValidateEmailDto)
  {
    ...
    if(!isValid) message = `The email address ${validateEmailDto.email} is invalid or not allowed.`;
    return {
