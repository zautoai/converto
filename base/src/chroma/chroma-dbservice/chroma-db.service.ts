...
import { EMBEDDING_PROVIDER_SBERT } from 'src/common/constants/system.constants';
...
export class ChromaDBService {
    ...
    async getClient() {
        const client = new ChromaClient({ path: process.env.CHROMA_URL , 
            auth: {provider: process.env.CHROMA_AUTH_PROVIDER, 
    ...
    async addDataTonamesapce(namespace: string, data: any, site: Site) {
        ...
        try {
            ...
            ? (await this.findSBERTVectors(sentances)) : (await this.findOpenAIVectors(sentances));
    ...
    async queryDocs(namespace: string, userMessage: string) {
        ...
        await this.findSBERTVectors([userMessage]) : await this.findOpenAIVectors([userMessage]);
