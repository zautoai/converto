...
export class FormBuilderService extends BaseService {
  constructor(private readonly formBuilderService: FormBuilderMicroService) {
    super();
  }

  async create(orgId: string, createFormBuilderDto: CreateFormBuilderDto) {
    return this.handleException(
      await this.formBuilderService.createForm(orgId, createFormBuilderDto),
    );
  }

  async findAll(orgId: string, filterDto: FilterDto) {
    return this.handleException(
      await this.formBuilderService.getForms(orgId, filterDto),
    );
  }

  async findOne(orgId: string, id: string) {
    return this.handleException(
      await this.formBuilderService.getForm(orgId, id),
    );
  }
  ...
  async update(
    ...
    return this.handleException(
      await this.formBuilderService.updateForm(orgId, id, updateFormBuilderDto),
    );
  }

  async remove(orgId: string, id: string) {
    return this.handleException(
      await this.formBuilderService.deleteForm(orgId, id),
    );
  }

  async generateFormScript(orgId: string, id: string) {
    return this.handleException(
      await this.formBuilderService.generateFormScript(orgId, id),
    );
  }

  async generateFormHTML(orgId: string, id: string) {
    return this.handleException(
      await this.formBuilderService.generateFormHTML(orgId, id),
    );
  }

  async submitForm(orgId: string, id: string, formData: any) {
    return this.handleException(
      await this.formBuilderService.submitForm(orgId, id, formData),
    );
  }
}
