...
import { CTA_SELECTOR_PROMPT } from 'src/common/templates/claude/cta-selector.template copy';
import { PAGE_SELECTOR_PROMPT } from 'src/common/templates/claude/page-selector.template copy';
...
export class CTASelectorService extends BaseService implements OnModuleInit {
  ...
  async handleMessage(channel: any, message: any) {
    try {
      ...
      const [content, ctaList, pageList] = await Promise.all([
        ...
        this.getCtaList({
          orgId,
          data: { types: [CTAType.CALENDAR, CTAType.CTA] },
        }),
      ...
      if (content.length >= 2) {
        this.generateStarters({
          orgId,
          data: { clientId: payload.clientId, content, agent },
        });
      }

      const ctaIds = [
        ...(await this.processContentForCTAs({
          orgId,
          data: { content, ctaList },
        })),
        ...(await this.processContentForCTAs({
          orgId,
          data: { content, ctaList: pageList },
          isCTA: false,
        })),
      ];

      if (ctaIds.length > 0) {
        const selectedCTAs = await prisma.callToAction.findMany({
          where: { id: { in: ctaIds } },
        });
        const ctaObjects = {
          ctas: selectedCTAs.filter(
            (cta) => cta.type === CTAType.CTA || cta.type === CTAType.CALENDAR,
          ),
          pageNavigators: selectedCTAs.filter(
            (cta) => cta.type === CTAType.NAVIGATOR,
          ),
        };
        this.redisPublisher.publish(
          'ctaselected',
          JSON.stringify({ clientId: payload.clientId, content: ctaObjects }),
        );
      }
    } catch (error) {
      console.error(error);
  ...
  async processContentForCTAs(
    serviceParams: ServiceParams<{ content; ctaList }>,
  ) {
    const {
      orgId,
      data: { content, ctaList },
    ...
    const result = isCTA
      ? await this.selectCTA(content, ctaList)
  ...
  async selectCTA(content: any, ctaList: any) {
    ...
    const promptMesssage = [{ role: 'system', content: prompt }];
    return await this.llmService.sendDirect(
      promptMesssage,
      LLMNames.COHERE,
  ...
  async selectPage(
    serviceParams: ServiceParams<{ content: any; pageList: any }>,
  ) {
    const {
      orgId,
      data: { content, pageList },
    } = serviceParams;
  ...
  async generateStarters(
    ...
    this.redisPublisher.publish(
      'ctaselected',
      JSON.stringify({ clientId, content: { starters: starterList } }),
    );
  }
}
