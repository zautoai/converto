...
export class LeadObsorverService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.redisSubscriber.on('message', async (channel, message) => {
      try {
        ...
        const agent = await this.agentService.findOne(payload.orgId, payload.agentId)
        const leadKeywords = agent.leadInfo.split(',');
        console.log(`Received message from ${channel}: ${message}`);
        const result = await this.findLead(content, leadKeywords,payload.orgId);
        console.log(result)
  ...
  getClassProperties(classType: any): string[] {
    const propertyNames = Object.getOwnPropertyNames(classType.prototype);
    return propertyNames.filter(name => {
      // Filter out methods and non-enumerable properties
      const propertyDescriptor = Object.getOwnPropertyDescriptor(classType.prototype, name);
      return propertyDescriptor && propertyDescriptor.enumerable && typeof propertyDescriptor.value !== 'function';
    });
  }

  async findLead(content: string, leadKeywords: string[],orgId:string) {
    let prompt = LEAD_OBSERVER_PROMPT_TEMPLATE;
    const response =await this.contactService.getContactFields(orgId)
    const contactFieldList = response.data;
    prompt = prompt.replaceAll('{{contactFieldList}}', contactFieldList.join(', '));
